  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between gap-1.5 bg-white border border-gray-200 rounded-md px-3 py-2 text-sm hover:border-blue-400 focus:outline-none focus:border-blue-500 transition-colors"
      >
        <span className={cn('truncate', allSelected ? 'text-gray-400' : 'text-gray-800')}>{label}</span>
        <div className="flex items-center gap-1 shrink-0">
          {!allSelected && (
            <X className="w-3.5 h-3.5 text-gray-400 hover:text-gray-600" onClick={(e) => { e.stopPropagation(); onChange([]) }} />
          )}
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-56 overflow-y-auto">
          <div
            className={cn('px-3 py-2.5 text-sm cursor-pointer hover:bg-blue-50 transition-colors', allSelected && 'font-semibold text-blue-700')}
            onClick={() => { onChange([]); setOpen(false) }}
          >
            Tutti
          </div>
          {options.map(opt => (
            <label key={opt} className="flex items-center gap-2.5 px-3 py-2 text-sm cursor-pointer hover:bg-gray-50 transition-colors">
              <input
                type="checkbox"
