...
export function DataTable() {
  ...
  const Th = ({ field, label }: { field: string; label: string }) => (
    <th className="text-left px-3 py-2 font-semibold cursor-pointer select-none whitespace-nowrap hover:bg-[#1a3f63]"
        onClick={() => sort(field)}>
      <span className="flex items-center gap-1">
        {label}
        {sortField === field
          ? sortDir === 1 ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />
          : <span className="text-[#9DC3E6]/40 text-xs">⇅</span>}
      </span>
    </th>
  )
    ...
    <div className="flex flex-col h-full">
      <div className="p-3 border-b border-gray-100 flex items-center gap-3">
        <input
          ...
          className="flex-1 border border-gray-200 rounded px-3 py-1.5 text-xs focus:outline-none focus:border-blue-500"
        />
        <span className="text-xs text-gray-400">{sorted.length.toLocaleString('it')} pratiche</span>
      </div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-[#f7fafd]'}>
                <td className="px-3 py-1.5 font-mono text-gray-600">{r['Nr Pratica']}</td>
                <td className="px-3 py-1.5 text-gray-500">{r.Esercizio}</td>
                <td className="px-3 py-1.5">{r['Data Inserimento']}</td>
                <td className="px-3 py-1.5">{r.Partenza}</td>
                <td className="px-3 py-1.5">{badgeStato(r.Stato)}</td>
                <td className="px-3 py-1.5">{r.Classificazione}</td>
                <td className="px-3 py-1.5">{r['Travel Consultant']}</td>
                <td className="px-3 py-1.5 max-w-[160px] truncate">{r['Agenzia Viaggi']}</td>
                <td className="px-3 py-1.5">{r['Area geografica']}</td>
                <td className="px-3 py-1.5 text-right font-medium">{fmt(r.Prezzo)}</td>
                <td className="px-3 py-1.5 text-right font-medium">{fmt(r.Incasso)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {pages > 1 && (
        <div className="p-3 border-t border-gray-100 flex items-center justify-center gap-2 text-xs text-gray-500">
